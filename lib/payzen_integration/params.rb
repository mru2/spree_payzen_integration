module PayzenIntegration
  ...
  class Params
    ...
    def self.check_returned_params(params)
      ...
      raise "Wrong return code : #{params[:vads_result]}" if params[:vads_result] != "00"  # code de retour, "00" = "tout s'est bien passé"
    ...
    def signature
      ...
      # get a hash with the params names and their value ; as strings
      hash_strings = {}
      PARAMS.each do |p|
        hash_strings[p.to_s] = self.send(p)
      end
          
      compute_signature(hash_strings)
    end
